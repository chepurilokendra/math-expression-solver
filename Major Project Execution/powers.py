...
def generator_main(co_ordinates,p,img_path,fresh_img):
    ...
    brace = cv2.imread("C:\\Users\sathw\Documents\Image segmentation\\brace.jpg")
    brace = cv2.cvtColor(brace,cv2.COLOR_BGR2RGB)
    print(square.shape)
    ...
    #     k=1
        ...
        elif ((i+1) in range(len(co_ordinates))) and abs((x+w) - (x_sq)) < 30 and abs((y+h) - y_sq) < 70:
            flag=1
            cv2.imwrite("C:\\Users\sathw\Documents\Image segmentation\equation\equation_"+str(p)+'\\seg_'+str(k+1)+".jpg",brace)
            cv2.imwrite("C:\\Users\sathw\Documents\Image segmentation\equation\equation_"+str(p)+'\\seg_'+str(k+2)+".jpg",brace)
            roi=img_s[co_ordinates[i+1][1]:co_ordinates[i+1][1]+co_ordinates[i+1][3],
                      co_ordinates[i+1][0]:co_ordinates[i+1][0]+co_ordinates[i+1][2]]
            cv2.rectangle(inp_s,(x_sq,y_sq),(x_sq+w_sq,y_sq+h_sq),(40,100,250),2)
            cv2.imwrite("C:\\Users\sathw\Documents\Image segmentation\equation\equation_"+str(p)+'\\seg_'+str(k+3)+".jpg",roi)
...
def solve_single(equations):
    ...
    if "=" in equations[0]:
        ...
        try:
            ...
            final_result=solution
            if len(str(solution))==0:
                solution=""
                solution =solution.join(equations[0])
            return equations,solution    
